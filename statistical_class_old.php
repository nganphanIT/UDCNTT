                ...
                <div class="group-box">
                    ...
                    if (!empty($_GET['nam'])) {
                        ...
                        while ($row = mysqli_fetch_array($result)) {
                            $nam = $row['nam'];
                            $sttkhoa = $row['sttkhoa'];
                            $sql1 = "SELECT * FROM lop where sttkhoa=$sttkhoa   GROUP BY matd;";
                            $result1 = mysqli_query($conn, $sql1);
                            while ($row1 = mysqli_fetch_array($result1)) {
                                $sttlop = $row1['sttlop'];
                                $matd = $row1['matd'];
                                $sttkhoa = $row1['sttkhoa'];
                                $sql2 = "SELECT   DISTINCT matd  FROM lop where sttkhoa=$sttkhoa and matd='$matd'   group BY matd";
                                $result2 = mysqli_query($conn, $sql2);
                                while ($row2 = mysqli_fetch_array($result2)) {
                                    $matd = $row2['0'];
                                    $sql2 = "SELECT   sttlop  FROM lop where sttkhoa=$sttkhoa and matd='$matd'";
                                    $result2 = mysqli_query($conn, $sql2);
                                    if ($result2) {
                                        while ($row3 = mysqli_fetch_array($result2)) {
                                            $sttlop = $row3[0];
                                            $sql4 = "select * from lop where sttlop = '$sttlop' ";
                                            $result4 = mysqli_query($conn, $sql4);
                                            while ($row4 = mysqli_fetch_array($result4)) {
                                                $matd = $row4['matd'];
                                                $sttkhoa = $row4['sttkhoa'];
                                                $sql5 = "select * from hocphi where matd='$matd' and sttkhoa = $sttkhoa";
                                            ...
                                            $sql6 = "select count (*) as num from hocvien where sttlop = '$sttlop' and tttt=1 ";
                                        ...
                                        //   $sql4= " SELECT hocphi.hpsv, lop.sttlop, lop.sttkhoa, lop.matd FROM hocphi INNER JOIN lop ON hocphi.sttkhoa=lop.sttkhoa";
                                        //   $result4 = mysqli_query($conn, $sql4);
